...
def bar_chart(base,pred):
    fig=go.Figure(go.Bar(
        ...
        marker_line_color=["#8892B0","#E8CC6A"],
        marker_line_width=1.2,
        text=[f"Rp {base:.1f}",f"Rp {pred:.1f}"],
        textposition="outside",
        textfont_color="#EEF0F7",
        width=0.38,
    ))
    fig.update_layout(
        paper_bgcolor=DARK,
        ...
        margin=dict(t=15,b=10,l=45,r=10),
        height=260,
        showlegend=False,
        bargap=.4
    )
...
st.markdown("<div class='footer'>Mohamad Yusuf - NPM 2313020037</div>",unsafe_allow_html=True)